...
http_3xx="s_^(3[^:]+): ([^ ]+) ([^ ]+ [^ ]+) (\\[.*\\]) ([^:]*):([^ ]*) ([^ ]+) \"([^\"]+)\" (.*)_${CYAN}\\1${END}: ${PURPLE}\\4${END} ${WHITE}\\2${END} ${ORANGE}\\3${END} ${LGREEN}http://\\5:\\6${END} \\7 ${ORANGE}\"\\8\"${END} \\9_";
http_xxx_443="s_^([^:]+): ([^ ]+) ([^ ]+ [^ ]+) (\\[.*\\]) ([^:]*):443 ([^ ]+) \"([^\"]+)\" (.*)_${RED}\\1${END}: ${PURPLE}\\4${END} ${WHITE}\\2${END} ${ORANGE}\\3${END} ${RED}https://\\5:443${END} \\6 ${ORANGE}\"\\7\"${END} \\8_";
...
#
# 2024-08-10 jj5 - NEW: new Dodo nbn address...
home="s_${ESC}\\[1;37m(139\\.216\\.80\\.100)${ESC}_${DGRAY}\\1${ESC}_";
